...
*/
    ...
    var selectElement = AKT.makeReprocessSelector(widget.element, widget.widgetName,
        'Formal term type', ['all','action','attribute','comparison','link','object','process','value'], 'all', 'term_type','formal_term_type');
    $(widgetSettings).append(selectElement);


    // Event handlers
    $(widgetContent).find('.listbox_types').on('change', function (event) {
        event.stopPropagation();
        console.log($(this).val());
        console.log('<<',$(widget.element).find('.content').find('.listbox_types'));
        var kbId = widget.options.kbId;
        var kb = AKT.KBs[kbId];
        //var type = $(widget.element).find('.content').find('.listbox_types').val();
        var type = $(this).val();
...
<div class="content" style="border:none; padding:10px; width:450px;">
    ...
    <div style="float:left;margin-top:7px;">
        <fieldset style="width:260px; margin-right:20px;">
            ...
            <select class="listbox_type_values" multiple size=13 style="width:240px; background:white;margin-top:10px;"></select>
            <button class="button_details" style="float:right;width:150px;height:25px;margin-top:5px;">Details for selected term</button>
        </fieldset>
    </div>
