...
AKT.widgets.hierarchies.setup = function (widget) {
    ...
    $(widget.element).find('.button_view').on('click', function (event,value) {   // Hierarchy view button
        ...
        if (widget.options.hierarchy_type === 'object') {
            var hierarchy = kb._objectHierarchies[hierarchyId];
        } else if (widget.options.hierarchy_type === 'topic') {
        ...
        var panel = AKT.panelise({
            ...
            options:{kbId:kbId, mode:'view', hierarchy:hierarchy}
        });
    ...
    $(widget.element).find('.button_new').on('click', function (event) {   // New Hierarchy button
        ...
        var panel = AKT.panelise({
            ...
            size:{width:'580px',height:'450px'},
            shift_key: event.shiftKey,
            options:{kbId:kbId, mode:'new', type:widget.options.hierarchy_type}
        });
    });
...
AKT.widgets.hierarchies.display = function (widget) {
    ...
    var treeType = widget.options.hierarchy_type;
    if (treeType === 'object') {
...
<div class="content" style="border:none;padding:15px;">
    ...
    <div class="w3-row">
        <div class="w3-col w3-right w3-container" style="width:120px">
            <fieldset>
            ...
            <fieldset>
                <legend style="font-size:13px;">Object</legend>
                <button class="button_details" style="width:70px; margin:3px;">Details</button>
            </fieldset>

        </div>

        <div class="w3-rest w3-container">
            <div class="div_hierarchies" style="height:500px;overflow-y:auto;">
        </div>
    </div>
