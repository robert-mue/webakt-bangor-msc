...
AKT.widgets.diagram.setup = function (widget) {
    ...
    var paper = new joint.dia.Paper({ el: $('#paper'), width: 880, height: 460, gridSize: 1, model: graph, linkPinning:false });
    ...
    $('#load_acheampong').on('click', function() {
        var kbId = AKT.state.current_kb;
        var kb = AKT.KBs[kbId];
        console.log(kb);
        var title = $('#load_diagram_local_storage_title').val();
        //var diagramString = localStorage.getItem(title);
        //var diagramObject = JSON.parse(diagramString);  TODO: fix this horrible temporary hack!


        //var diagramObject1= kb._kb_from_file.diagrams.acheampong;
        //var diagramObject = kb._oldDiagrams.acheampong;
        //var diagramObject1 = kb._diagrams.acheampong._joint;  // The original JointJs version
        //var diagramObject = kb._diagrams.acheampong.joint;  // The JointJs version generated from the Systo version

        var diagram = new Diagram('acheampong_from_topic','systo');
        // Confusingly, Chromolaena is the topic name for acheampong.
        diagram.convertCausalToSysto('Chromolaena');
    ...
    $('#show_acheampong').on('click', function() {
        var diagram = AKT.state.current_diagram;  // The object, not the ID

        console.log(diagram);

        var diagramObject = diagram.convertSystoToJoint();
        //var diagramObject = kb._diagrams.acheampong_from_causal;
        //console.log(diagramObject.nodes[0]);
        //console.log(diagramObject.links[0]);
        //console.log('--------------------------------');
        //console.log(diagramObject1.nodes[0]);
        //console.log(diagramObject1.links[0]);

        var nodes = diagramObject.nodes;
        for (var i=0; i<nodes.length; i++) {
            var node = new joint.shapes.standard.Rectangle(nodes[i]);
                node.on('change:position', function(element, position) {
                    console.log('-->> ',i,'node change:position');
...
function updateMidnodes(element){
    _.each(graph.getLinks(), function(link) {
        ...
        if (element.id === sourceId || element.id === targetId) {
            ...
            //if (link.attributes.mytype === 'influence') {  // TODO: webakt arc behaviour
            if (true) {  // TODO: webakt arc behaviour
                // TODO: This is the bit that's not executed if link...mytype is not set; see above.
                console.debug('-----');
...
<div class="content" style="border:none;padding:5px;">
    ...
    <div class="main_div w3-row">
        ...
        <!-- =========== diagram div =================== -->
        <div id="paper" class="w3-rest w3-container paper" style="background:white; width:500px; height:500px;"></div>
