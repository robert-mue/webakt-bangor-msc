(function ($) {
  ...
  /***********************************************************
   *         dialog_HierarchyCollapsibleTree widget
   ***********************************************************
   */
    $.widget('akt.dialog_HierarchyCollapsibleTree', {
        meta:{
            short_description: 'dialog_HierarchyCollapsibleTree',
            long_description: 'dialog_HierarchyCollapsibleTree',
        ...
        widgetEventPrefix: 'dialog_HierarchyCollapsibleTree:',

        _create: function () {
            console.debug('\n### Creating instance of widget "akt.dialog_HierarchyCollapsibleTree".',
                '\nElement ID:',this.element[0].id,'  Widget UUID:',this.uuid,'  Options:',this.options);
            var self = this;
            this.element.addClass('dialog_HierarchyCollapsibleTree-1');
        ...
        _destroy: function () {
            this.element.removeClass('dialog_HierarchyCollapsibleTree-1');
            this.element.empty();
            this._super();
        },
    ...
    function evaluate(kbId) {
        console.debug('Starting akt.dialog_HierarchyCollapsibleTree: evaluate()...');
        var results = null;
        return results;
    }
    ...
    function display(widget, results) {
        console.debug('Starting akt.dialog_HierarchyCollapsibleTree: display()');
        console.debug(results);
        var kbId = AKT.state.current_kb;
        var kb = AKT.kbs[kbId];

        if (widget.options.show_titlebar) {
            var widgetTitlebar = $('<div class="titlebar">dialog_HierarchyCollapsibleTree<input type="button" value="X" class="dialog_close_button"/></div>');  
        ...
        var zindex = AKT.incrementZindex("dialog_HierarchyCollapsibleTree");
